...
-- Fix the inventory update trigger to completely remove any updated_at field reference
-- The issue is that the trigger tries to reference NEW.updated_at but this field
-- is not accessible in the NEW record during the UPDATE operation

DROP TRIGGER IF EXISTS update_inventory_after_checkout_trigger ON public.inventory_transactions;

-- Recreate the trigger function with zero references to updated_at from NEW record
...
BEGIN
  ...
  IF NEW.type = 'check_out' AND NEW.delivery_status = 'delivered' AND 
    ...
    IF item_id IS NULL THEN
      ...
      -- Log the error in activity_logs (no updated_at reference)
    ...
    ELSE
      ...
      -- Build detailed log information (completely removed NEW.updated_at reference)
...
IS 'Reduces inventory quantities when checkout transactions are marked as delivered - completely fixed version without any updated_at field references';

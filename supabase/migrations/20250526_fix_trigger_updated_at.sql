...
-- Fix the inventory update trigger to remove any reference to updated_at field
-- which doesn't exist in the inventory_transactions table

DROP TRIGGER IF EXISTS update_inventory_after_checkout_trigger ON public.inventory_transactions;

-- Recreate the trigger function without any reference to updated_at

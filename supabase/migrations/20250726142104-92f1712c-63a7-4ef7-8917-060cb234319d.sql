-- Phase 1: Fix remaining critical RLS policies (corrected for actual table structure)
...
-- Enable RLS on system_modules table (if it exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'system_modules') THEN
    ...
    IF EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'system_modules' AND column_name = 'is_active') THEN
    ...
    ELSE
      ...
      CREATE POLICY "Users can view system modules" 
      ON public.system_modules 
      FOR SELECT 
      USING (true);
    END IF;
...
-- Enable RLS on role_permissions table (if it exists)
DO $$
BEGIN

...
DROP POLICY IF EXISTS "Authenticated users can view tax types" ON public.tax_types;
CREATE POLICY "Authenticated users can view tax types"
ON public.tax_types FOR SELECT
TO authenticated
USING (true);

DROP POLICY IF EXISTS "Admins can manage tax types" ON public.tax_types;
...
DROP TRIGGER IF EXISTS update_tax_types_updated_at_trigger ON public.tax_types;
...
-- Drop and recreate the get_applicable_tax_code function with new return columns
